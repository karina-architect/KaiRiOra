...
export function getHeroInfographic(locale: Locale): HeroInfographicContent {
  return {
    ...
    imageAlt: t(locale, {
      en: "Two data specialists analysing live analytics dashboards on a large screen in a modern technology operations centre",
      es: "Dos especialistas en datos analizan paneles de analítica en tiempo real en una gran pantalla de un centro tecnológico moderno",
      pt: "Dois especialistas em dados analisam painéis de analítica em tempo real num ecrã grande de um centro tecnológico moderno",
      fr: "Deux spécialistes des données analysent des tableaux de bord analytiques en temps réel sur un grand écran dans un centre technologique moderne",
      de: "Zwei Datenspezialisten analysieren Echtzeit-Analyse-Dashboards auf einem großen Bildschirm in einem modernen Technologiezentrum",
      ru: "Два специалиста по данным анализируют аналитические панели в реальном времени на большом экране в современном технологическом центре",
      hu: "Két adatspecialista valós idejű analitikai irányítópultokat elemez egy nagy kijelzőn egy modern technológiai központban",
    }),
    startLabel: t(locale, {
      en: "Start with",
      es: "Empieza con",
